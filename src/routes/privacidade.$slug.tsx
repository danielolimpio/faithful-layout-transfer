...
export const Route = createFileRoute("/privacidade/$slug")({
  loader: ({ params }) => {
    const a = articlesBySlug[params.slug];
    if (!a || a.categoryHref !== "/privacidade") throw notFound();
    return { article: a };
  },
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          ...
          { name: "description", content: loaderData.article.blocks.find((b) => "text" in b && b.type === "p")?.["text" as never] as unknown as string || loaderData.article.title },
